...
Constellation (inter-agent):
    POST /constellation/aura         → receive aura from a peer agent (emotional contagion)
    GET  /constellation/state        → snapshot of own + all registered peer auras
...
@app.post("/constellation/aura")
def receive_constellation_aura(
    payload: dict = Body(...),
    _: None = Depends(require_auth),
):
    """Receive an aura broadcast from a peer agent.

    Applies weighted emotional contagion via AURA → LIMBIC.
    ...
    try:
        ...
        logger.warning(f"Constellation aura receive error: {exc}")
        raise HTTPException(status_code=500, detail=f"Failed to apply emotional contagion: {exc}")


@app.get("/constellation/state")
def get_constellation_state(_: None = Depends(require_auth)):
    """Snapshot of own aura + all registered peer auras.

    Returns own current aura and the last received aura from each registered peer,
    ...
    try:
        ...
        # Fallback: read raw constellation file
        const_data = _read_json("constellation.json", {})
        aura_data = _read_json("aura.json", {})
        peers = const_data.get("peers", {})
        last_received = const_data.get("last_received", {})
        peer_states = {}
        for name, info in peers.items():
            received = last_received.get(name, {})
            elapsed = None
            if received.get("received_at"):
                elapsed = round(time.time() - received["received_at"], 1)
            peer_states[name] = {
                "aura": received.get("aura"),
