...
class HealthServer:
    ...
    async def _handle_feedback(self, request: web.Request) -> web.Response:
        ...
        POST /feedback
        {
            ...
            "outcome": "success" | "partial" | "blocked" | "cascade_stop", // how it went
            "summary": "Wrote journal entry, updated goals",  // what happened
            "decay_overrides": {"goals": 0.8, "curiosity": 0.3}  // optional per-drive decay
        }
        
        cascade_stop: GENERATE cycled the same low-priority task 3+ times — all genuine work
        complete. Decays ALL drives fully regardless of drives_addressed list, preventing
        re-triggering when there is nothing real to do.
        ...
        # cascade_stop: GENERATE cycled the same task 3+ times — all genuine work complete.
        # Decay ALL drives fully to prevent re-triggering when there is nothing to do.
        if outcome == "cascade_stop":
            for drive_name, drive in self.daemon.drives.drives.items():
                before = drive.pressure
                decay_amount = drive.pressure  # full decay
