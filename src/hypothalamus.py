...
def _load_state() -> dict:
    ...
    if _DEFAULT_STATE_FILE.exists():
        try:
            ...
            # Migration: handle old schema with "need_signals"/"retired" keys
            if "need_signals" in raw and "pending_signals" not in raw:
                raw["pending_signals"] = {}
            if "retired" in raw and "retired_drives" not in raw:
                raw["retired_drives"] = raw.pop("retired", [])
            if "generated_drives" in raw:
                del raw["generated_drives"]
            # Ensure all required keys exist with correct types
            if not isinstance(raw.get("pending_signals"), dict):
                raw["pending_signals"] = {}
            if not isinstance(raw.get("active_drives"), dict):
                raw["active_drives"] = {}
            if not isinstance(raw.get("retired_drives"), list):
                raw["retired_drives"] = []
            return raw
