...
class DriveEngine:
    ...
    def _apply_sensor_spikes(self, sensor_data: dict):
        ...
        if system_alerts:
            ...
            # No system alerts → passively decay system drive back toward baseline.
            # This prevents pressure from staying pinned at max_pressure after a
            # transient alert (e.g., memory_pressure spike that self-resolves).
            # Decay rate: 0.1 per tick (roughly 0.5/min at 5s tick intervals), floor 0.
            if "system" in self.drives and self.drives["system"].pressure > 0:
                self.drives["system"].decay(0.1)
                if self.drives["system"].pressure == 0:
                    logger.debug("System drive fully decayed — no active alerts")
