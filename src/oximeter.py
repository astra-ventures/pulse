...
def update_metrics(followers: int = None, likes: int = None, replies: int = None, sentiment: float = None) -> dict:
    """Update external metrics. Falls back to x-stats.json if no direct values provided."""
    state = _load_state()
    m = state["metrics"]

    # Fallback: read from x-stats.json if any metric is still at zero default
    _x_stats_path = Path.home() / ".openclaw" / "workspace" / "memory" / "self" / "x-stats.json"
    if _x_stats_path.exists():
        try:
            _x = json.loads(_x_stats_path.read_text())
            if followers is None and m.get("followers", 0) == 0:
                followers = _x.get("followers")
            if likes is None and m.get("likes", 0) == 0:
                likes = _x.get("likes_per_post_avg")
            if replies is None and m.get("replies", 0) == 0:
                replies = _x.get("replies_per_post_avg")
            if sentiment is None and m.get("sentiment", 0.5) == 0.5:
                sentiment = _x.get("sentiment")
        except (json.JSONDecodeError, OSError):
            pass
