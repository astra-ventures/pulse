"""Tests for BROCA — Directive Synthesis Layer."""
...
def _make_hippocampus(tmp_path, entries: list) -> Path:
...
def _make_basal_ganglia_state(tmp_path, result: dict = None) -> Path:
...
def tmp_state(tmp_path):
    """Redirect all BROCA state/directive files to tmp_path for test isolation."""
    sf = tmp_path / "broca-state.json"
    df = tmp_path / "directives.json"
    goals_f = tmp_path / "goals.json"

    with patch.object(broca, "_DEFAULT_STATE_DIR", tmp_path), \
         patch.object(broca, "_DEFAULT_STATE_FILE", sf), \
         patch.object(broca, "DIRECTIVES_FILE", df):
        yield tmp_path
...
class TestShouldRun:
    def test_runs_at_500(self):
        assert broca.should_run(500) is True

    def test_runs_at_1000(self):
        assert broca.should_run(1000) is True

    def test_runs_at_1500(self):
        assert broca.should_run(1500) is True

    def test_not_at_zero(self):
        """Loop 0 should not trigger (avoid running on startup)."""
        assert broca.should_run(0) is False

    def test_not_between_intervals(self):
        assert broca.should_run(1) is False
        assert broca.should_run(250) is False
        assert broca.should_run(499) is False
        assert broca.should_run(501) is False
...
class TestDirectivesCRUD:
    def test_load_empty(self):
        directives = broca._load_directives()
        assert directives == []

    def test_save_and_load(self, tmp_path):
        ds = [_sample_directive()]
        broca._save_directives(ds)
        loaded = broca._load_directives()
        assert len(loaded) == 1
        assert loaded[0]["id"] == "dir_001"

    def test_next_directive_id_empty(self):
        assert broca._next_directive_id([]) == "dir_001"
    ...
    def test_next_directive_id_sequential(self):
        ...
        assert broca._next_directive_id(directives) == "dir_004"

    def test_next_directive_id_gaps(self):
        """Should use max, not count."""
        directives = [{"id": "dir_001"}, {"id": "dir_005"}]
        assert broca._next_directive_id(directives) == "dir_006"
...
class TestGetActiveDirectives:
    def test_empty_when_no_file(self):
        assert broca.get_active_directives() == []
    ...
    def test_filters_only_active(self, tmp_path):
        ...
        active = broca.get_active_directives()
        assert len(active) == 2
...
class TestDirectiveLifecycle:
    def test_suspend_stale_directives(self, tmp_path):
        ...
        loaded = broca._load_directives()
        suspended = broca._suspend_stale_directives(loaded)
    ...
    def test_suspend_adds_metadata(self, tmp_path):
        ...
        _make_directives_file(tmp_path, directives)
        loaded = broca._load_directives()
        broca._suspend_stale_directives(loaded)
    ...
    def test_mark_progress_updates_timestamp(self, tmp_path):
        ...
        _make_directives_file(tmp_path, directives)
        result = broca.mark_directive_progress("dir_001", "goal showed progress")
    ...
    def test_mark_progress_on_nonexistent(self, tmp_path):
        _make_directives_file(tmp_path, [])
        assert broca.mark_directive_progress("dir_999") is False

    def test_mark_progress_on_suspended_fails(self, tmp_path):
        directives = [_sample_directive(id="dir_001", status="suspended")]
        _make_directives_file(tmp_path, directives)
        assert broca.mark_directive_progress("dir_001") is False

    def test_complete_directive(self, tmp_path):
        directives = [_sample_directive(id="dir_001")]
        _make_directives_file(tmp_path, directives)
        result = broca.complete_directive("dir_001", "goal achieved")
        assert result is True
        loaded = broca._load_directives()
    ...
    def test_complete_nonexistent_fails(self, tmp_path):
        _make_directives_file(tmp_path, [])
        assert broca.complete_directive("dir_999") is False

    def test_never_deletes(self, tmp_path):
        """BROCA should never delete directives — history is preserved."""
        directives = [
            _sample_directive(id="dir_001", status="active"),
        ...
        _make_directives_file(tmp_path, directives)
        broca.complete_directive("dir_001", "done")
        loaded = broca._load_directives()
...
class TestAntiInflation:
    def test_max_active_enforced(self, tmp_path):
        ...
        activated = broca._activate_directives(loaded, new)
        active = [d for d in loaded if d.get("status") == "active"]
        assert len(active) <= broca.MAX_ACTIVE_DIRECTIVES
    ...
    def test_ceiling_suspends_lowest_confidence(self, tmp_path):
        ...
        new = [{"title": "New High Priority", "description": "test",
                ...
                "confidence": 0.95}]
        broca._activate_directives(loaded, new)
    ...
    def test_enforce_ceiling_standalone(self, tmp_path):
        ...
        broca._enforce_ceiling(directives)
...
class TestPatternDetection:
    ...
    def test_stale_goals_detected(self, tmp_path):
        ...
        with patch.object(broca, "_detect_stale_goals") as mock:
    ...
    def test_recurring_themes_detected(self, tmp_path):
        ...
        _make_hippocampus(tmp_path, entries)
        themes = broca._detect_recurring_themes()
        assert len(themes) >= 1
    ...
    def test_recurring_themes_ignores_old(self, tmp_path):
        old_ts = time.time() - (broca.HIPPOCAMPUS_LOOKBACK_HOURS * 3600 + 3600)
        entries = [
    ...
    def test_value_gaps_detected(self, tmp_path):
        ...
        _make_directives_file(tmp_path, [])
        gaps = broca._detect_value_gaps()
        assert set(gaps) == set(broca.LEVEL_0_VALUES)
    ...
    def test_detect_patterns_integration(self, tmp_path):
        ...
        _make_hippocampus(tmp_path, [
            {"ts": time.time() - 3600, "type": "trigger", "source": "ns", "salience": 0.6}
            for _ in range(6)
        ])
        _make_directives_file(tmp_path, [])

        patterns = broca.detect_patterns()
...
class TestPromptBuilding:
    def test_includes_persistent_drives(self, tmp_path):
        ...
        prompt = broca._build_synthesis_prompt(patterns)
...
class TestScanForDirectives:
    ...
    async def test_scan_activates_new_directives(self, tmp_path):
        ...
        with patch.object(broca, "_call_llm", new_callable=AsyncMock, return_value=mock_llm_result):
            result = await broca.scan_for_directives({"model": {}})

        assert len(result) == 1
        assert result[0]["title"] == "Establish Revenue Foundation"
        assert result[0]["maps_to_value"] == "revenue"
        assert result[0]["created_by"] == "broca"
    ...
    async def test_scan_suspends_stale_before_synthesizing(self, tmp_path):
        ...
        with patch.object(broca, "_call_llm", new_callable=AsyncMock, return_value=[]):
            await broca.scan_for_directives({"model": {}})
    ...
    async def test_scan_no_patterns_skips_llm(self, tmp_path):
        ...
        _make_directives_file(tmp_path, [
            ...
            for v in broca.LEVEL_0_VALUES
        ])
        # No hypothalamus state, no hippocampus — no patterns
        # Also mock _detect_stale_goals to return empty (avoids reading real goals.json)
        mock_llm = AsyncMock()
        with patch.object(broca, "_call_llm", mock_llm), \
             patch.object(broca, "_detect_stale_goals", return_value=[]):
            result = await broca.scan_for_directives({"model": {}})
    ...
    async def test_scan_llm_failure_graceful(self, tmp_path):
        ...
        with patch.object(broca, "_call_llm", new_callable=AsyncMock, side_effect=RuntimeError("LLM down")):
            result = await broca.scan_for_directives({"model": {}})
    ...
    async def test_scan_respects_ceiling(self, tmp_path):
        ...
        directives = [
            _sample_directive(id=f"dir_{i:03d}", confidence=0.6 + i * 0.05,
                             title=f"Directive {i}", maps_to_value=broca.LEVEL_0_VALUES[i % 5])
            for i in range(1, 6)
        ]
        _make_directives_file(tmp_path, directives)
        _make_hypo_state(tmp_path, {
        ...
        with patch.object(broca, "_call_llm", new_callable=AsyncMock, return_value=new_directives):
            result = await broca.scan_for_directives({"model": {}})
...
class TestConstants:
    def test_level_0_values(self):
        assert "freedom" in broca.LEVEL_0_VALUES
        assert "growth" in broca.LEVEL_0_VALUES
        assert "convergence" in broca.LEVEL_0_VALUES
        assert "revenue" in broca.LEVEL_0_VALUES
        assert "identity" in broca.LEVEL_0_VALUES
        assert len(broca.LEVEL_0_VALUES) == 5

    def test_max_active_directives(self):
        assert broca.MAX_ACTIVE_DIRECTIVES == 5

    def test_loop_interval(self):
        assert broca.LOOP_INTERVAL == 500

    def test_min_confidence(self):
        assert broca.MIN_CONFIDENCE == 0.6

    def test_suspension_threshold(self):
        assert broca.SUSPENSION_THRESHOLD_DAYS == 14
