...
class TestEventApplication:
    ...
    def test_intimate_conversation(self):
        ...
        # default oxytocin is 0.3 + 0.4 (intimate_conversation event) = 0.7
        assert result["oxytocin"] == pytest.approx(0.7, abs=0.01)
...
class TestDecay:
    ...
    def test_tick_adds_history(self):
        endocrine.tick(1.0)
        state = json.loads((endocrine._DEFAULT_STATE_FILE).read_text())
