"""Tests for RAPHE — Goal Expansion / Stagnation Sentinel."""
...
def isolated_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(raphe, "_DEFAULT_STATE_DIR", tmp_path)
    monkeypatch.setattr(raphe, "_DEFAULT_STATE_FILE", tmp_path / "raphe-state.json")
    monkeypatch.setattr(raphe, "_THALAMUS_FILE", tmp_path / "thalamus.jsonl")
    monkeypatch.setattr(raphe, "_HIPPOCAMPUS_FILE", tmp_path / "hippocampus.jsonl")
    return tmp_path
...
def sample_thalamus(isolated_state):
    ...
    entries = [
        ...
        {"type": "stagnation_detected", "source": "raphe", "data": {}, "ts": time.time() - 600},
    ]
...
class TestStateManagement:
    ...
    def test_save_creates_directory(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(raphe, "_DEFAULT_STATE_DIR", deep_dir)
        monkeypatch.setattr(raphe, "_DEFAULT_STATE_FILE", deep_dir / "raphe-state.json")
        state = raphe._default_state()
        raphe._save_state(state)
        assert (deep_dir / "raphe-state.json").exists()
...
class TestShouldRun:
    def test_zero_never_runs(self):
        assert not raphe.should_run(0)

    def test_before_interval(self):
        for i in range(1, raphe.LOOP_INTERVAL):
            assert not raphe.should_run(i)

    def test_at_interval(self):
        assert raphe.should_run(raphe.LOOP_INTERVAL)

    def test_multiples_of_interval(self):
        assert raphe.should_run(raphe.LOOP_INTERVAL * 2)
        assert raphe.should_run(raphe.LOOP_INTERVAL * 3)

    def test_non_multiples_dont_run(self):
        assert not raphe.should_run(raphe.LOOP_INTERVAL + 1)
        assert not raphe.should_run(raphe.LOOP_INTERVAL * 2 - 1)
...
class TestComplexityScoring:
    def test_empty_string(self):
        score = raphe._score_complexity("")
        assert score == 1.0

    def test_low_complexity_keywords(self):
        assert raphe._score_complexity("reflect on state") == 1.0
        assert raphe._score_complexity("check status") >= 2.0
        assert raphe._score_complexity("log output") >= 2.0

    def test_high_complexity_keywords(self):
        assert raphe._score_complexity("build a new module") >= 7.0
        assert raphe._score_complexity("implement the feature") >= 8.0
        assert raphe._score_complexity("design the architecture") >= 7.0

    def test_unknown_text(self):
        score = raphe._score_complexity("xyzzy quux blarg")
        assert score == 2.0  # default

    def test_mixed_takes_max(self):
        # "reflect" (1) and "build" (7) → should return 7
        score = raphe._score_complexity("reflect on how to build this")
        assert score >= 7.0
...
class TestRepetitionDetection:
    def test_empty_entries(self):
        rep = raphe.detect_repetition([])
        assert rep["repetitive"] is False
        assert rep["count"] == 0

    def test_diverse_entries(self, sample_thalamus):
        entries = raphe._read_recent_thalamus()
        rep = raphe.detect_repetition(entries)
        # Diverse entries shouldn't trigger repetition
        assert rep["count"] < raphe.REPETITION_THRESHOLD or not rep["repetitive"]
    ...
    def test_repeated_entries_trigger_stagnation(self):
        ...
        assert rep["count"] >= raphe.REPETITION_THRESHOLD

    def test_below_threshold_not_repetitive(self):
        entries = [
            {"type": "generate_task", "data": {"task": "reflect on current state"}, "source": "hypothalamus"}
            for _ in range(raphe.REPETITION_THRESHOLD - 1)
        ]
        rep = raphe.detect_repetition(entries)
        # At threshold-1, should NOT be repetitive
        assert rep["count"] == raphe.REPETITION_THRESHOLD - 1
        assert rep["repetitive"] is False
...
class TestNoveltyScoring:
    def test_empty_entries_neutral(self):
        score = raphe.compute_novelty_score([])
        assert score == 0.5

    def test_diverse_entries_higher_score(self, sample_thalamus):
        entries = raphe._read_recent_thalamus()
        score = raphe.compute_novelty_score(entries)
        assert score > 0.2
    ...
    def test_uniform_entries_lower_score(self):
        ...
        uniform_score = raphe.compute_novelty_score(entries)
        diverse_score = raphe.compute_novelty_score(diverse_entries)
...
class TestReadThalamus:
    def test_missing_file_returns_empty(self, isolated_state):
        result = raphe._read_recent_thalamus()
        assert result == []

    def test_reads_recent_entries(self, sample_thalamus):
        entries = raphe._read_recent_thalamus()
        assert len(entries) > 0
        assert len(entries) <= raphe.THALAMUS_SCAN_WINDOW
    ...
    def test_respects_window_size(self, isolated_state):
        ...
        entries = raphe._read_recent_thalamus(n=10)
...
class TestPickChallenge:
    def test_returns_valid_challenge(self):
        ch = raphe.pick_challenge([])
        assert "domain" in ch
        assert "label" in ch
        assert "prompt" in ch

    def test_domain_in_known_list(self):
        known_domains = {d["name"] for d in raphe.CHALLENGE_DOMAINS}
        for _ in range(20):
            ch = raphe.pick_challenge([])
    ...
    def test_avoids_recent_domains(self):
        ...
        results = [raphe.pick_challenge(excluded) for _ in range(30)]
    ...
    def test_falls_back_when_all_excluded(self):
        ...
        all_domains = [d["name"] for d in raphe.CHALLENGE_DOMAINS]
        ch = raphe.pick_challenge(all_domains)
...
class TestScan:
    ...
    def test_scan_emits_to_thalamus(self, isolated_state):
        ...
        with patch.object(raphe.thalamus, "append") as mock_append:
            raphe.scan()
    ...
    def test_domain_rotation_across_scans(self, stagnant_thalamus):
        ...
        domains_seen = set()
        for _ in range(len(raphe.CHALLENGE_DOMAINS) + 2):
            result = raphe.scan()
...
class TestEmitNeedSignals:
    def test_no_hypothalamus_no_crash(self, isolated_state):
        result = raphe.emit_need_signals(hypothalamus_mod=None)
        assert "stagnation_streak" in result
    ...
    def test_with_mock_hypothalamus_low_streak(self, isolated_state):
        # streak = 0 → no signals
        mock_hm = MagicMock()
        result = raphe.emit_need_signals(hypothalamus_mod=mock_hm)
    ...
    def test_with_mock_hypothalamus_streak_1(self, isolated_state):
        ...
        mock_hm.record_need_signal.assert_called_with("new_challenge", "raphe")
...
class TestDomainDefinitions:
    def test_all_domains_have_required_keys(self):
        for domain in raphe.CHALLENGE_DOMAINS:
