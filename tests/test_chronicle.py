...
class TestQuerySince:
    ...
    def test_returns_recent_entries(self):
        chronicle.record_event("test", "recent", {"note": "now"}, salience=0.6)
        results = chronicle.query_since(hours=1)
        assert len(results) == 1

    def test_excludes_old_entries(self):
        import time
        # Record an entry then manually backdate it
        chronicle.record_event("test", "old", {}, salience=0.6)
        # query_since(0) should return nothing (cutoff = now)
        results = chronicle.query_since(hours=0)
        assert results == []

    def test_empty_file(self):
        results = chronicle.query_since(hours=24)
        assert results == []

    def test_returns_all_within_window(self):
        for i in range(3):
            chronicle.record_event("src", f"ev{i}", {}, salience=0.7)
        results = chronicle.query_since(hours=1)
        assert len(results) == 3

    def test_oldest_first_ordering(self):
        chronicle.record_event("a", "first", {}, salience=0.6)
        chronicle.record_event("b", "second", {}, salience=0.6)
        results = chronicle.query_since(hours=1)
