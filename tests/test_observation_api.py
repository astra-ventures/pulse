...
class TestConstellation:
    """Tests for POST /constellation/aura and GET /constellation/state."""
    ...
    def test_receive_aura_negative_mood(self, client, state_dir, monkeypatch):
        ...
        payload = {
            "source_agent": "mira",
            "timestamp": 1740412800.0,
            "aura": {"mood": "burned_out", "energy": 0.1},
        }
        r = client.post("/constellation/aura", json=payload)
        assert r.status_code == 200
    ...
    def test_receive_aura_all_constellation_agents(self, client, state_dir, monkeypatch):
        ...
        for agent in ["iris", "vera", "mira", "sage", "lyra"]:
            ...
            r = client.post("/constellation/aura", json=payload)
            assert r.status_code == 200, f"Failed for agent: {agent}"
    ...
    def test_constellation_state_returns_200(self, client, state_dir, monkeypatch):
        """GET /constellation/state returns 200."""
        # Write a minimal constellation file so fallback reads cleanly
        (state_dir / "constellation.json").write_text('{"peers":{}, "own_name":"iris", "last_received":{}}')
        (state_dir / "aura.json").write_text('{"mood":"energized","energy":0.9,"available":true,"focus":0.7,"social_battery":0.8,"last_emit":1740412800.0}')
        r = client.get("/constellation/state")
        assert r.status_code == 200

    def test_constellation_state_has_own_aura(self, client, state_dir, monkeypatch):
        ...
        r = client.get("/constellation/state")
        data = r.json()
        assert "own_aura" in data or "own_name" in data
    ...
    def test_constellation_state_has_peers(self, client, state_dir, monkeypatch):
        ...
        monkeypatch.setattr(aura_mod, "get_constellation_state", lambda: {
            ...
            "peers": {
                "vera": {"aura": {"mood": "content"}, "elapsed_seconds": 30.0, "weight": 0.5},
            },
            "timestamp": 1740412800.0,
        })
        r = client.get("/constellation/state")
    ...
    def test_constellation_state_own_name(self, client, state_dir, monkeypatch):
        ...
        r = client.get("/constellation/state")
        data = r.json()
        assert data.get("own_name") == "iris"

    def test_receive_aura_requires_auth_when_token_set(self, auth_client, monkeypatch):
        """POST /constellation/aura requires auth when token is configured."""
        r = auth_client.post("/constellation/aura", json=self.SAMPLE_AURA_PAYLOAD)
        assert r.status_code == 401
    ...
    def test_receive_aura_accepts_auth_token(self, auth_client, monkeypatch):
        ...
        r = auth_client.post(
            "/constellation/aura",
            json=self.SAMPLE_AURA_PAYLOAD,
            headers={"Authorization": "Bearer test-token-123"},
        )
        assert r.status_code == 200

    def test_constellation_state_requires_auth_when_token_set(self, auth_client, monkeypatch):
        """GET /constellation/state requires auth when token is configured."""
        r = auth_client.get("/constellation/state")
        assert r.status_code == 401
