...
class TestEncode:
    def test_basic_encode(self):
        eg = encode(
            ...
            {"valence": 0.8, "intensity": 0.9, "label": "joy"},
            "main_session",
        )
    ...
    def test_encode_persists(self):
        encode(
            "Event 1", {"valence": 0.5, "intensity": 0.5, "label": "calm"}, "discord"
        )
        store = _load_store()
        assert len(store) == 1
    ...
    def test_encode_auto_associates(self):
        ...
        eg2 = encode(
            "Second", {"valence": 0, "intensity": 0.3, "label": "neutral"}, "dream"
        )
    ...
    def test_encode_with_sensory(self):
        eg = encode(
            "Voice call",
            {"valence": 0.9, "intensity": 0.8, "label": "warmth"},
            "main_session",
    ...
    def test_encode_default_timestamp(self):
        eg = encode(
            "Now", {"valence": 0, "intensity": 0.5, "label": "neutral"}, "cron_session"
        )
    ...
    def test_encode_custom_timestamp(self):
        eg = encode(
            "Past",
            {"valence": 0, "intensity": 0.5, "label": "neutral"},
            "cron_session",
            timestamp=1000.0,
        )
...
class TestRecall:
    def test_recall_by_keyword(self):
        encode(
            "Wrote a poem about stars",
            {"valence": 0.7, "intensity": 0.6, "label": "creative"},
            "main_session",
        )
        encode(
            "Fixed a bug in spine",
            {"valence": 0.3, "intensity": 0.4, "label": "satisfaction"},
            "cron_session",
        )
    ...
    def test_recall_returns_string(self):
        encode(
            "Testing recall format",
            {"valence": 0.5, "intensity": 0.5, "label": "focus"},
            "main_session",
        )
...
class TestRecallByPlace:
    def test_place_recall(self):
        encode(
            "Discord chat",
            {"valence": 0.5, "intensity": 0.5, "label": "social"},
            "discord",
        )
        encode(
            "Main session work",
...
class TestRecallByEmotion:
    def test_emotion_filter(self):
        encode(
            "Happy moment",
            {"valence": 0.9, "intensity": 0.8, "label": "joy"},
            "main_session",
        )
        encode(
            "Sad moment",
            {"valence": -0.8, "intensity": 0.7, "label": "sadness"},
            "main_session",
        )
    ...
    def test_emotion_filter_intensity(self):
        encode(
            "Low intensity",
            {"valence": 0.5, "intensity": 0.2, "label": "calm"},
            "main_session",
        )
        encode(
            "High intensity",
            {"valence": 0.5, "intensity": 0.9, "label": "excitement"},
            "main_session",
        )
...
class TestRecallByTime:
    def test_time_window(self):
        encode(
            "Old",
            {"valence": 0, "intensity": 0.5, "label": "n"},
            "main_session",
            timestamp=1000.0,
        )
        encode(
...
class TestConsolidate:
    def test_consolidation(self):
        eg1 = encode(
            "Wrote code",
            {"valence": 0.5, "intensity": 0.6, "label": "focus"},
            "main_session",
        )
        eg2 = encode(
            "Dreamed of ocean",
            {"valence": 0.7, "intensity": 0.5, "label": "peace"},
            "dream",
        )
...
class TestPrune:
    def test_prune_removes_low_intensity(self):
        for i in range(15):
            encode(
                f"E{i}",
                {"valence": 0, "intensity": i * 0.06, "label": "n"},
                "main_session",
            )
    ...
    def test_prune_noop_under_max(self):
        encode(
            "Only one", {"valence": 0, "intensity": 0.5, "label": "n"}, "main_session"
        )
        prune(max_entries=10)
        store = _load_store()
        assert len(store) == 1
...
class TestRecallWeighted:
    ...
    def test_recall_keyword_match(self):
        ...
        encode(
            "Built a new API endpoint",
            {"valence": 0.5, "intensity": 0.5, "label": "focus"},
            "main_session",
        )
        encode(
            "Went for a dream walk",
            {"valence": 0.7, "intensity": 0.3, "label": "calm"},
            "dream",
        )
    ...
    def test_recall_importance_weighted(self):
        ...
        encode(
            "Deploy server alpha",
            {"valence": 0.5, "intensity": 0.2, "label": "focus"},
            "cron_session",
            timestamp=now,
        )
        encode(
            "Deploy server beta",
            {"valence": 0.5, "intensity": 0.9, "label": "focus"},
            "cron_session",
            timestamp=now,
        )
    ...
    def test_recall_raw_returns_dicts(self):
        ...
        encode(
            ...
            {"valence": 0.6, "intensity": 0.7, "label": "curiosity"},
            "main_session",
        )
...
class TestEngramDataclass:
    def test_roundtrip(self):
        eg = Engram(
            id="test",
            event="hello",
            emotion={"valence": 0, "intensity": 0.5, "label": "n"},
