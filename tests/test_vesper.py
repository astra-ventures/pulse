"""Tests for VESPER — Nightly Restoration / Synthesis."""
...
from pulse.src import vesper


@pytest.fixture(autouse=True)
def tmp_state(tmp_path):
    sf = tmp_path / "vesper-state.json"
    synth_dir = tmp_path / "daily-synthesis"
    with patch.object(vesper, "_DEFAULT_STATE_DIR", tmp_path), \
         patch.object(vesper, "_DEFAULT_STATE_FILE", sf), \
         patch.object(vesper, "_DAILY_SYNTH_DIR", synth_dir):
        yield tmp_path


class TestShouldRun:
    def test_not_deep_night(self):
        assert vesper.should_run("morning", 300) is False

    def test_wrong_loop_count(self):
        assert vesper.should_run("deep_night", 301) is False

    def test_deep_night_correct_loop_no_history(self):
        # No history → last_run=0 → always eligible
        assert vesper.should_run("deep_night", 300) is True
    ...
    def test_too_recent(self, tmp_path):
        ...
        assert vesper.should_run("deep_night", 300) is False
...
class TestRunRestoration:
    ...
    def test_calls_endocrine_decay(self):
        mock_endo = MagicMock()
        vesper.run_restoration(endocrine_mod=mock_endo)
        mock_endo.tick.assert_called_once_with(hours=1.0)

    def test_rem_success_fired_on_meaningful_events(self, tmp_path):
        # Inject a chronicle mod that returns many shipped entries
        mock_chronicle = MagicMock()
        mock_chronicle.query_by_date.return_value = [
        ...
        mock_endo = MagicMock()
        result = vesper.run_restoration(chronicle_mod=mock_chronicle, endocrine_mod=mock_endo)
    ...
    def test_reads_memory_file(self, tmp_path):
        ...
        result = vesper.run_restoration(memory_dir=tmp_path)
...
class TestGetLastSynthesis:
    def test_empty_when_no_files(self):
        result = vesper.get_last_synthesis()
        assert result == {}
