"""Tests for PREFRONTAL — Runtime Identity Enforcement."""
...
def temp_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(prefrontal, "_DEFAULT_STATE_DIR", tmp_path)
    monkeypatch.setattr(prefrontal, "_DEFAULT_STATE_FILE", tmp_path / "prefrontal-state.json")
    yield tmp_path
...
class TestScanResponse:
    ...
    def test_clean_response_scores_high(self):
        ...
        result = prefrontal.scan_response(text)
    ...
    def test_source_tracked_in_history(self):
        prefrontal.scan_response("Test message", source="test_session")
        state = json.loads((prefrontal._DEFAULT_STATE_FILE).read_text())
        assert any(r["source"] == "test_session" for r in state["compliance_history"])

    def test_checks_run_increments(self):
        for _ in range(3):
            prefrontal.scan_response("hello")
        state = json.loads(prefrontal._DEFAULT_STATE_FILE.read_text())
        assert state["checks_run"] == 3

    def test_compliance_history_capped_at_200(self):
        for i in range(210):
            prefrontal.scan_response(f"message {i}")
        state = json.loads(prefrontal._DEFAULT_STATE_FILE.read_text())
...
class TestGetStatus:
    ...
    def test_status_after_clean_scans(self):
        for _ in range(5):
            prefrontal.scan_response("I've got this. Let me handle it. 💜")
        status = prefrontal.get_status()
        assert status["status"] == "healthy"
    ...
    def test_recent_avg_reflects_last_10(self):
        ...
        for _ in range(10):
            prefrontal.scan_response("I've got this. 💜")
        prefrontal.scan_response("As an AI, I'm unable to help.")
        status = prefrontal.get_status()
...
class TestAmygdalaThreat:
    ...
    def test_severe_drift_returns_threat(self):
        threat = prefrontal.amygdala_threat("drift_severe")
        assert threat is not None
        assert threat["source"] == "PREFRONTAL"
        assert threat["intensity"] == 0.8
        assert "severe" in threat["type"]

    def test_moderate_drift_returns_lower_threat(self):
        threat = prefrontal.amygdala_threat("drift_moderate")
        assert threat is not None
        assert threat["intensity"] == 0.4

    def test_minor_drift_returns_none(self):
        assert prefrontal.amygdala_threat("drift_minor") is None

    def test_clean_returns_none(self):
        assert prefrontal.amygdala_threat("clean") is None
...
class TestInjectSoulContext:
    ...
    def test_inject_with_existing_file(self, tmp_path):
        ...
        result = prefrontal.inject_soul_context(soul_path=soul)
        assert "IDENTITY ANCHOR" in result
        assert "I am Iris" in result

    def test_inject_with_missing_file(self, tmp_path):
        result = prefrontal.inject_soul_context(soul_path=tmp_path / "nonexistent.md")
        assert result == ""
...
class TestComplianceTrend:
    ...
    def test_returns_empty_initially(self):
        trend = prefrontal.get_compliance_trend()
        assert trend == []

    def test_returns_last_n_records(self):
        for _ in range(25):
            prefrontal.scan_response("I've got this.")
        trend = prefrontal.get_compliance_trend(n=10)
        assert len(trend) == 10
