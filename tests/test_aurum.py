"""Tests for AURUM — Financial Nervous System."""
...
from pulse.src import aurum


@pytest.fixture(autouse=True)
def tmp_state(tmp_path):
    sf = tmp_path / "aurum-state.json"
    # Point GOALS_FILE to a nonexistent path so tests don't read real goals
    dummy_goals = tmp_path / "goals.json"
    with patch.object(aurum, "_DEFAULT_STATE_DIR", tmp_path), \
         patch.object(aurum, "_DEFAULT_STATE_FILE", sf), \
         patch.object(aurum, "_GOALS_FILE", dummy_goals):
        yield tmp_path


class TestUpdateBalance:
    def test_update_monthly_revenue(self):
        state = aurum.update_balance("monthly_revenue", 5000.0)
        assert state["monthly_revenue"] == 5000.0

    def test_update_polymarket(self):
        state = aurum.update_balance("polymarket_balance", 250.0)
        assert state["polymarket_balance"] == 250.0

    def test_history_logged(self, tmp_path):
        aurum.update_balance("monthly_revenue", 1000.0)
        sf = tmp_path / "aurum-state.json"
    ...
    def test_invalid_field_raises(self):
        with pytest.raises(ValueError, match="Unknown balance field"):
            aurum.update_balance("invalid_field", 100.0)

    def test_history_capped_at_200(self):
        for i in range(205):
            aurum.update_balance("trading_pnl_7d", float(i))
        sf = tmp_path / "aurum-state.json" if False else None
        state = aurum._load_state()
...
class TestGetFinancialPressure:
    def test_zero_revenue_max_pressure(self):
        ...
        pressure = aurum.get_financial_pressure()
        assert pressure == 1.0

    def test_full_revenue_no_pressure(self):
        aurum.update_balance("monthly_revenue", 20000.0)
        pressure = aurum.get_financial_pressure()
        assert pressure == 0.0

    def test_half_revenue_half_pressure(self):
        aurum.update_balance("monthly_revenue", 10000.0)
        pressure = aurum.get_financial_pressure()
        assert abs(pressure - 0.5) < 0.01

    def test_over_target_no_pressure(self):
        aurum.update_balance("monthly_revenue", 25000.0)
        pressure = aurum.get_financial_pressure()
        assert pressure == 0.0

    def test_pressure_linear(self):
        aurum.update_balance("monthly_revenue", 5000.0)
        p1 = aurum.get_financial_pressure()
        aurum.update_balance("monthly_revenue", 15000.0)
        p2 = aurum.get_financial_pressure()
        assert p2 < p1


class TestGetStatus:
    def test_status_fields(self):
        status = aurum.get_status()
    ...
    def test_revenue_gap(self):
        aurum.update_balance("monthly_revenue", 8000.0)
        status = aurum.get_status()
...
class TestEmitNeedSignals:
    def test_high_pressure_emits_signal(self):
        ...
        mock_hypo = MagicMock()
        mock_endo = MagicMock()
        result = aurum.emit_need_signals(hypothalamus_mod=mock_hypo, endocrine_mod=mock_endo)
    ...
    def test_low_pressure_no_signals(self):
        aurum.update_balance("monthly_revenue", 18000.0)
        mock_hypo = MagicMock()
        mock_endo = MagicMock()
        result = aurum.emit_need_signals(hypothalamus_mod=mock_hypo, endocrine_mod=mock_endo)
    ...
    def test_no_modules_ok(self):
        ...
        result = aurum.emit_need_signals()
